...
class User private constructor(
    ...
    private val salt: String by lazy {
        ByteArray(LENGTH_OF_BYTE_ARRAY).also {
    ...
    fun changePassword(oldPassword: String, newPassword: String) {
        if (encrypt(oldPassword) == passwordHash) {
    ...
    private fun generateAccessCode() = StringBuilder().apply {
        repeat(TIMES_TO_REPEAT) {
            (POSSIBLE_SYMBOLS.indices).random().also { index ->
                append(POSSIBLE_SYMBOLS[index])
            }
        }
    }.toString()
